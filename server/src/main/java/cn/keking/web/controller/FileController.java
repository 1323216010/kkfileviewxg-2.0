...
import cn.keking.utils.MethodUtils;
import cn.keking.utils.StaticVariables;
...
public class FileController {
    ...
    public String fileUpload(@RequestParam("file") MultipartFile file) throws JsonProcessingException {
        ...
        try(InputStream in = file.getInputStream()) {
            if(in.available() <= 0){
                ...
                cn.keking.pojo.File file1 = new cn.keking.pojo.File();
                file1.setPath(fileDir + demoPath + fileName);
                file1.setName(fileName);
                file1.setType(fileType);
                String url = StaticVariables.path + fileName;
    ...
    public String deleteFile(String fileName, HttpServletRequest request) throws JsonProcessingException {
        ...
        if (file.exists() && !file.delete()) {
           logger.error("删除文件【{}】失败，请检查目录权限！，IP：{}",file.getPath(),ip);
            ...
            StaticGetPrivate.getTemplates().getForObject(StaticVariables.deletePath + fileName, String.class);
        }
    ...
    public String fileUpload2(@RequestParam("file") MultipartFile file) throws JsonProcessingException {
        if (ConfigConstants.getFileUploadDisable()) {
            return new ObjectMapper().writeValueAsString(ReturnResponse.failure("文件传接口已禁用"));
        }
        // 获取文件名
